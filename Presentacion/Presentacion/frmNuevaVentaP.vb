...
Public Class frmNuevaVentaP
    ...
    Private Sub btnFinalizar_Click(sender As Object, e As EventArgs) Handles btnFinalizar.Click
        ...
        If dtSer.Rows.Count = 0 Then
            ...
            If verificarPartidaDoble() = True Then
                'If Decimal.Parse(txtTotal2.Text.Trim) = Decimal.Parse(txtTotalVenta.Text.Trim) Then
                procesarBoton()
                'Else
                'MsgBox("El total del comprobante no es igual al monto de la cuenta de la venta")
                'End If
            End If
        Else
        ...
        If dgvOperaciones.RowCount > 0 Then
            ...
            For Each row As DataGridViewRow In dgvOperaciones.Rows
                If row.Cells("num_cuenta").Value.ToString.StartsWith("12") Then
                    total = total + Decimal.Parse(row.Cells("debe").Value.ToString)
                    Exit For
                End If
            Next
    ...
    Private Sub cboPercepcion_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cboPercepcion.SelectedIndexChanged
        If iCarga = 1 Then
            Dim dataPorc As New DataTable
            dataPorc = objCrud.nCrudListarBD("select * from impuestos_sunat where tipo_comprobante='VENTA' and  id='" & cboPercepcion.SelectedValue.ToString & "'", CadenaConexion)
            If dataPorc.Rows.Count > 0 Then
                txtPorcPercep.Text = dataPorc.Rows(0)("porcentaje").ToString
