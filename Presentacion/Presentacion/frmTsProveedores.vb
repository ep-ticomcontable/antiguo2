...
Public Class frmTsProveedores
    ...
    Private Sub cargarDatos()
        ...
        data = obj.nCrudListarBD("select * from vista_proveedores order by nombre asc", CadenaConexion)
    ...
    Private Sub cargarDatosCuenta()
        If tipo <> "new" Or tipo = "" Then
            ...
            Dim sql As String
            sql = "select * from proveedores where id=" & codigoCeldaSeleccionada()
            Dim data As New DataTable
            data = obj.nCrudListarBD(sql, CadenaConexion)
            With data
                txtDni.Text = .Rows(0)("dni_ruc")
    ...
    Private Sub txtDato_TextChanged(sender As Object, e As EventArgs) Handles txtDato.TextChanged
        ...
        data = obj.nCrudListarBD("select * from vista_proveedores where nombre like '%" & dato & "%' or dni_ruc like '%" & dato & "%' order by nombre,dni_ruc asc", CadenaConexion)
        dgvLista.DataSource = data
        cargaInicialEstados()
        'End If
    End Sub
