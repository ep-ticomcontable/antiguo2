...
Public Class frmListaLetrasPorCobrar
    ...
    Private Sub cargarComprobantes()
        Dim data As New DataTable
        data = obj.nCrudListarBD("select * from vista_listaLetrasVentas order by fec_emision asc", CadenaConexion)
    ...
    Private Sub btnPagarConLetra_Click(sender As Object, e As EventArgs) Handles btnPagarLetra.Click
        If iCarga = 1 Then
            ...
            With dgvLista.Rows(f)
                ...
                data = obj.nCrudListarBD("select IsNull(sum(monto), 0)  as 'abono' from abono_pagos_ventas where id_venta='" & .Cells("id_comprobante").Value & "'", CadenaConexion)
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles btnModificar.Click
        If dgvLista.Rows.Count > 0 Then
            ...
            frmModificarLetraPorPagar.tipo = "VENTA"
    ...
    Private Sub dgvLista_SelectionChanged(sender As Object, e As EventArgs) Handles dgvLista.SelectionChanged
        ...
        If dgvLista.Rows(f).Cells("estado_deuda").Value.ToString.StartsWith("DEBE") Then
            ...
            Dim dt As New DataTable
            dt = obj.nCrudListarBD("select IsNull(sum(monto), 0)  as 'abono' from abono_pagos_ventas where id_venta='" & dgvLista.Rows(f).Cells("id_comprobante").Value & "'", CadenaConexion)
