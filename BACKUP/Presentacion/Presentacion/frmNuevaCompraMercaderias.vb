...
Public Class frmNuevaCompraMercaderias
    ...
    Private Sub guardarDatos(estadoComprobante As String)
        ...
        Else
            ...
            With entiCCompra
                ...
                .marca = IIf(cboTipoCompra.SelectedValue = "CREDITO", cboImpuesto.SelectedValue & "@" & IIf(cboImpuesto.Text = "SIN AFECTO", "0", "0"), "0" & "@I")
            ...
            If cboImpuesto.Text = "PERCEPCIÓN" Then
                ...
                dataPorc = objCrud.nCrudListar("select * from impuestos_sunat where tipo_comprobante='COMPRA' and id='" & cboImpuesto.SelectedValue.ToString & "'")
                ...
                For Each row As DataGridViewRow In dgvOperaciones.Rows
                    If dataPorc.Rows(0)("cuenta").ToString = row.Cells("num_cuenta").Value.ToString Then
                        With entiImpuesto
                            ...
                            .serie = "0" 'cboSerieImpuesto.Text.Trim
            ...
            With entiCCompraAsiento
                ...
                .numero_detraccion = txtNumeroImpuesto.Text.Trim
                ...
                .valor_detraccion = Decimal.Round((Decimal.Parse(txtTotalCompra.Text.Trim) * Decimal.Parse(txtPorcPercep.Text.Trim)), 2)
    ...
    Private Sub agregarCuentaContable()
        ...
        Dim dataIgv As New DataTable
        dataIgv = objCrud.nCrudListarBD("select * from igv where estado=1", CadenaConexion)
        monto = IIf(cboOperacion.SelectedValue.ToString = "1", (Format(total / (1 + ((dataIgv.Rows(0)("valor").ToString) / 100)), "#,##0.00")), "0.00")
        ...
        cuentas.Rows.Add(dataIgv.Rows(0)("cuenta").ToString, obtenerDatosCuenta(dataIgv.Rows(0)("cuenta").ToString), (total - monto), "0.00", txtGlosa.Text.Trim)
        'para los impuestos'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
        ...
        If cboImpuesto.SelectedValue <> "0" Then
            ...
            cuentas.Rows.Add(dataIC.Rows(0)("cuenta").ToString, obtenerDatosCuenta(dataIC.Rows(0)("cuenta").ToString), "0.00", calculoI, IIf(txtNumeroImpuesto.Enabled = True, "N°: " & txtNumeroImpuesto.Text, "SUJETO A DETRACCIÓN"))
    ...
    Private Sub txtPorcPercep_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtPorcPercep.KeyPress
        If Asc(e.KeyChar) = 13 Then
            txtNumeroImpuesto.Focus()
        End If
    End Sub
    Private Sub txtNumeroImpuesto_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtNumeroImpuesto.KeyPress
        If Asc(e.KeyChar) = 13 Then
