...
Public Class frmListaComprobanteCompras
    ...
    Private Sub btnPagos_Click(sender As Object, e As EventArgs) Handles btnPagos.Click
        frmAccionPagos.idComprobante = "0"
        frmAccionPagos.idComprobante = codigoComprobante()
        frmAccionPagos.Show()
    End Sub
    ...
    Private Sub btnCredito_Click(sender As Object, e As EventArgs) Handles btnCredito.Click
        With frmNuevaNotaCreditoCompra
    ...
    Private Sub btnCanje_Click(sender As Object, e As EventArgs) Handles btnCanje.Click
        Dim f As Integer
        f = dgvLista.CurrentCell.RowIndex() 'obtiene la fila seleccionada del Grid
        If dgvLista.Rows(f).Cells("tipo_compra").Value.ToString = "LETRA POR PAGAR" And Decimal.Parse(dgvLista.Rows(f).Cells("abono").Value.ToString) <= Decimal.Parse(dgvLista.Rows(f).Cells("total").Value.ToString) Then
            lblTitulo.Text = "COMPRAS"
            grupoBusqueda.Enabled = False
            ...
            'frmDetalleCompra.idCompra = codigoComprobante()
            'frmDetalleCompra.Show()
        ElseIf dgvLista.Rows(f).Cells("tipo_compra").Value.ToString <> "LETRA POR PAGAR" And Decimal.Parse(dgvLista.Rows(f).Cells("abono").Value.ToString) <= Decimal.Parse(dgvLista.Rows(f).Cells("total").Value.ToString) Then
            realizarCanje()
        Else
    ...
    Private Sub generarExcelCompras()
        ...
        Try
            ...
            a.Visible = True
            Archivo = "D:\excel\formato.8.1-copia.xlsx" 'Pon aqui el nombre de archivo que quieras
    ...
    Private Sub generarExcelComprasPle()
        ...
        Try
            ...
            a = CreateObject("Excel.Application")
            a.Visible = True
            Archivo = "D:\excel\registro.compras.xlsx" 'Pon aqui el nombre de archivo que quieras
            APrueba = a.Workbooks.Open(Archivo)
            ...
            With APrueba.WorkSheets("formato")
                ...
                For i As Integer = 0 To trKardex - 1
                    ...
                    .Cells((fila + i), "F") = ""
                    Dim tc As DataTable = obj.nCrudListar("select * from tipo_documento where id='" & tbDatos.Rows(i)("id_tipo_comprobante").ToString & "'")
    ...
    Private Sub generarTxt()
        ...
        Try
            ...
            If Directory.Exists("D:\excel\PLE\") = False Then ' si no existe la carpeta se crea
                Directory.CreateDirectory("D:\excel\PLE\")
            End If

            Windows.Forms.Cursor.Current = Cursors.WaitCursor
            PathArchivo = "D:\excel\PLE\" & nomArchivo & ".txt" ' Se determina el nombre del archivo con la fecha actual
    ...
    Private Sub btnAcciones_Click(sender As Object, e As EventArgs)
        Dim f As Integer
        f = dgvLista.CurrentCell.RowIndex() 'obtiene la fila seleccionada del Grid
        frmAccionesCompras.idComprobante = dgvLista.Rows(f).Cells("id").Value
        frmAccionesCompras.idTipoComprobante = dgvLista.Rows(f).Cells("id_tipo_comprobante").Value
        frmAccionesCompras.montoAbono = dgvLista.Rows(f).Cells("abono").Value
        frmAccionesCompras.ShowDialog()
    End Sub
    Private Sub btnActualizar_Click(sender As Object, e As EventArgs) Handles btnActualizar.Click
        realizarConsulta()
    End Sub
    Private Sub dgvLetras_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvLetras.CellClick
        Dim f As Integer
        f = dgvLetras.CurrentCell.RowIndex() 'obtiene la fila seleccionada del Grid

        Dim dtLetras As New DataTable
        dtLetras = obj.nCrudListarBD("SELECT cuenta as 'cuentaL',glosa as 'glosaL',debe as 'debeL',haber as 'haberL' FROM asientos_libro_diario where id_comprobante='LC" & dgvLetras.Rows(f).Cells("idL").Value & "' order by id asc", CadenaConexion)
        dgvCuentasLetras.DataSource = dtLetras
    ...
    Private Sub realizarCanje()
        ...
        If txtTotalCanjes.Text.Trim <= txtDeuda2.Text.Trim Then
            With frmCanjeLetraComprobantes
                .Text = "Canje de Comprobante de COMPRA por Letra"
                .tipo = "COMPRA"
                .codComprobante = codigoComprobante()
