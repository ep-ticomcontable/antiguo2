...
Public Class frmListaLetrasACobrar
    ...
    Private Sub cargarComprobantes()
        Dim data As New DataTable
        data = obj.nCrudListarBD("usp_listarLetrasPorCobrar", CadenaConexion)
    ...
    Private Sub btnPagarConLetra_Click(sender As Object, e As EventArgs) Handles btnPagarConLetra.Click
        If iCarga = 1 Then
            Dim f As Integer
            ...
            For Each fila As DataGridViewRow In dgvLista.Rows
                With fila
                    If .Cells(0).Value = True Then
                        Dim data As New DataTable
                        data = obj.nCrudListarBD("select IsNull(sum(monto), 0)  as 'abono' from abono_pagos_ventas where id_venta='" & .Cells("id").Value & "'", CadenaConexion)

                        Dim diferencia As Decimal = 0
                        Dim pagado As Decimal = 0
                        If data.Rows(0)("abono").ToString.Length > 0 Then
                            diferencia = (Decimal.Parse(.Cells("monto").Value) - Decimal.Parse(data.Rows(0)("abono").ToString))
                            pagado = Decimal.Parse(data.Rows(0)("abono").ToString)
                        End If
                        dataComp.Rows.Add(.Cells("id").Value, .Cells("serie").Value, .Cells("numero").Value, .Cells("librado").Value, .Cells("monto").Value, pagado, diferencia, diferencia, Date.Parse(.Cells("fec_vencimiento").Value).ToString("dd/MM/yyyy"))
                    End If
                End With
            Next

            With frmAbonoDeudasLetrasACobrar
                .Text = "Pago de Letras - Múltiple"
