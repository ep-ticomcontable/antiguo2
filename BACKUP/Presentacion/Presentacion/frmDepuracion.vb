...
Public Class frmDepuracion
    ...
    Private Sub cargarClientes()
        ...
        data = obj.nCrudListar("select id,dni,nombre,ape_paterno,ape_materno,ruc,razon_social,nom_comercial,direccion from clientes order by nombre asc")
    ...
    Private Sub cargarClientesRepetidos()
        ...
        Dim data As New DataTable
        data = obj.nCrudListar("select id,dni,nombre,ape_paterno,ape_materno,ruc,razon_social,nom_comercial,direccion from clientes where (nombre like '" & dgvCab.Rows(f).Cells("nombre").Value.ToString.Trim & "%' or dni like '%" & dgvCab.Rows(f).Cells("id").Value.ToString.Trim & "%') and id<>'" & dgvCab.Rows(f).Cells("id").Value.ToString.Trim & "' order by nombre asc")
