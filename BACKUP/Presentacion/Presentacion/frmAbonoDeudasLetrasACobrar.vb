...
Public Class frmAbonoDeudasLetrasACobrar
    ...
    Private Sub cargarListaBancos()
        ...
        Try
            data2 = obj.nCrudListar("select * from bancos where estado=1 order by nombre asc")
        ...
        For Each row As DataGridViewRow In dgvLista.Rows
            If Decimal.Parse(row.Cells("debe").Value) > 0 Then
                ...
                If cboTipo.SelectedValue.ToString <> "EF" Then
                    Dim data As New DataTable
                    data = obj.nCrudListar("select * from abono_pagos_ventas where id_venta='" & row.Cells("id_venta").Value.ToString & "' order by id desc")
