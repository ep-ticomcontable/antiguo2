...
Public Class frmListaRegistroLibroDiario
    ...
    Private Sub cargarDatos()
        ...
        query = "select id,id_comprobante,fecha_operacion,glosa,cuenta,denominacion,debe,haber from asientos_libro_diario  where 1=1 order by id asc"
        ...
        For Each row As DataRow In data.Rows
            With row
                If .Item("id_comprobante").ToString.StartsWith("A") And Not .Item("id_comprobante").ToString.StartsWith("ABC") And Not .Item("id_comprobante").ToString.StartsWith("ABV") Then
                    ...
                    'MsgBox(.Item("id_comprobante").ToString & "#" & .Item("id").ToString)
                    'listaArray.Add(.Item("id_comprobante").ToString & "#" & .Item("id").ToString)
                    'contador = contador + 1
                ElseIf .Item("id_comprobante").ToString.StartsWith("C") And Not .Item("id_comprobante").ToString.StartsWith("CAJA") Then
                    cuentas.Rows.Add(.Item("id_comprobante").ToString, "-", "-", "ASIENTO DE COMPRAS", .Item("id").ToString, "-", "0", "0")
                    cuentas.Rows.Add(.Item("id").ToString, .Item("id_comprobante").ToString, Date.Parse(.Item("fecha_operacion")).ToString("dd/MM/yyyy"), .Item("glosa").ToString, .Item("cuenta").ToString, obtenerDatosCuenta(.Item("cuenta").ToString), .Item("debe").ToString, .Item("haber").ToString)
                    'For i As Integer = 0 To cuentas.Rows.Count - 1
                    '    If cuentas.Rows(i)("id").ToString.StartsWith("C") And cuentas.Rows(i)("glosa").ToString = "ASIENTO DE COMPRAS" And Not cuentas.Rows(i)("id").ToString.StartsWith("CAJA") Then
                    '        cuentas.Rows(i)("cuenta") = cuentas.Rows(i)("cuenta") & ","
                    '    End If
                    'Next
                ...
                ElseIf .Item("id_comprobante").ToString.StartsWith("RLD") Then
                    ...
                    cuentas.Rows.Add(.Item("id").ToString, .Item("id_comprobante").ToString, Date.Parse(.Item("fecha_operacion")).ToString("dd/MM/yyyy"), .Item("glosa").ToString, .Item("cuenta").ToString, obtenerDatosCuenta(.Item("cuenta").ToString), .Item("debe").ToString, .Item("haber").ToString)
                ElseIf .Item("id_comprobante").ToString.StartsWith("LC") Then
                    cuentas.Rows.Add(.Item("id_comprobante").ToString, "-", "-", "CANJE LETRA COMPRA", .Item("id").ToString, "-", "0", "0")
                    cuentas.Rows.Add(.Item("id").ToString, .Item("id_comprobante").ToString, Date.Parse(.Item("fecha_operacion")).ToString("dd/MM/yyyy"), .Item("glosa").ToString, .Item("cuenta").ToString, obtenerDatosCuenta(.Item("cuenta").ToString), .Item("debe").ToString, .Item("haber").ToString)
                ElseIf .Item("id_comprobante").ToString.StartsWith("LV") Then
