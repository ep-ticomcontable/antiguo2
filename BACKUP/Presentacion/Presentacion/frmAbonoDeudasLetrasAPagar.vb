Imports Negocio
Imports Entidades
Public Class frmAbonoDeudasLetrasAPagar
        ...
        If e.KeyChar = Convert.ToChar(Keys.Enter) Then
            If txtCuenta.Text.Trim.Length >= 2 Then
                With frmEscogerPlanContable
                    .formInicio = "frmAbonoDeudasLetras"
        ...
        For Each row As DataGridViewRow In dgvLista.Rows
            If Decimal.Parse(row.Cells("debe").Value) > 0 Then
                With entiAbono
                    ...
                    .id_venta = row.Cells("id_compra").Value.ToString
                    ...
                    .estado = "1"
                End With
                MsgBox("ABONO: " & objAbono.nRegistrarAbonoVentasBD(entiAbono, CadenaConexion))
                If cboTipo.SelectedValue.ToString <> "EF" Then
                    Dim data As New DataTable
                    data = obj.nCrudListar("select * from abono_pagos_compras where id_compra='" & row.Cells("id_compra").Value.ToString & "' order by id desc")
                    With entiConciliacion
                        ...
                        .estado = "1"
                    End With
                    MsgBox("REGISTRO CONCILIACION: " & objCon.registrarConciliacionBD(entiConciliacion, CadenaConexion))
                End If
            End If
            With entiLD
            ...
            End With
            MsgBox("REGISTRO LIBRO DIARIO: " & objLD.registrarAsientoLibroDiarioBD(entiLD, CadenaConexion))
        Next
    End Sub
