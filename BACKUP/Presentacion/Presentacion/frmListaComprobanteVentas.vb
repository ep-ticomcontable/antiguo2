...
Public Class frmListaComprobanteVentas
    ...
    Private Function querysPorCombinacion() As String
        ...
        query = "select * from vista_ventas_con_abonos where 1=1 "
    ...
    Private Sub generarExcelVentasPle()
        ...
        Try
            ...
            a = CreateObject("Excel.Application")
            a.Visible = True
            Archivo = "D:\excel\registro.ventas.xlsx" 'Pon aqui el nombre de archivo que quieras
            APrueba = a.Workbooks.Open(Archivo)
    ...
    Private Sub generarTxt()
        ...
        nomArchivo = "LE" & data.Rows(0)("ruc").ToString & "2016" & mesPeriodo & "00" & codPLEVenta & "001111"
    ...
    Private Sub btnPagos_Click(sender As Object, e As EventArgs) Handles btnPagos.Click
        frmAccionPagosVentas.idComprobante = "0"
        frmAccionPagosVentas.idComprobante = codigoComprobante()
        frmAccionPagosVentas.ShowDialog()
    End Sub
    ...
    Private Sub generarExcelVentas()
        ...
        Try
            ...
            a = CreateObject("Excel.Application")
            a.Visible = True
            Archivo = "D:\excel\formato.14.1 - copia.xlsx" 'Pon aqui el nombre de archivo que quieras
            APrueba = a.Workbooks.Open(Archivo)
            With APrueba.WorkSheets("Hoja1")
                .Cells(3, "F") = cboMes.Text
                .Cells(4, "F") = data.Rows(0)("ruc").ToString
    ...
    Private Sub btnCanje_Click(sender As Object, e As EventArgs) Handles btnCanje.Click
        Dim f As Integer
        f = dgvLista.CurrentCell.RowIndex() 'obtiene la fila seleccionada del Grid
        If dgvLista.Rows(f).Cells("tipo_venta").Value.ToString = "CANJE" And Decimal.Parse(dgvLista.Rows(f).Cells("abono").Value.ToString) < Decimal.Parse(dgvLista.Rows(f).Cells("total").Value.ToString) Then
            lblTitulo.Text = "VENTAS"
            grupoBusqueda.Enabled = False
        ...
        ElseIf dgvLista.Rows(f).Cells("tipo_venta").Value.ToString <> "CANJE" And Decimal.Parse(dgvLista.Rows(f).Cells("abono").Value.ToString) < Decimal.Parse(dgvLista.Rows(f).Cells("total").Value.ToString) Then
            realizarCanje()
        Else
    ...
    Private Sub realizarCanje()
        ...
        With frmCanjeLetraComprobantes
            .Text = "Canje de Comprobante de VENTA por Letra"
            .tipo = "VENTA"
