...
Public Class frmParametroLibroMayor
    ...
    Private Sub generarExcel()
        ...
        Try
            ...
            a.Visible = True
            Archivo = "D:\excel\formato.hoja.trabajo.xlsx" 'Pon aqui el nombre de archivo que quieras
            ...
            With APrueba.WorkSheets("formato")
                ...
                For i As Integer = 0 To trKardex - 1
                    APrueba.Worksheets("formato").Rows(indicador + i).Insert(Shift:=(indicador + i))
                    ...
                    If ctaPadre = "70" Then
                        .Cells((indicador + i), "N") = .Cells((indicador + i), "D").Value
            ...
            'objE.Visible = True
            'ArchivoEstado = "D:\excel\formato.hoja.trabajo.xlsx" 'Pon aqui el nombre de archivo que quieras
            'objExcel = objE.Workbooks.Open(ArchivoEstado)
            With APrueba.WorkSheets("estado_perdida_ganancia")
    ...
    Private Sub btnVerLibroMayor_Click(sender As Object, e As EventArgs) Handles btnVerLibroMayor.Click
        ...
        a = CreateObject("Excel.Application")
        a.Visible = True
        Archivo = "D:\excel\formato.6.1.xlsx" 'Pon aqui el nombre de archivo que quieras
        APrueba = a.Workbooks.Open(Archivo)
    ...
    Private Sub btnGeneral_Click(sender As Object, e As EventArgs) Handles btnGeneral.Click
        ...
        a = CreateObject("Excel.Application")
        a.Visible = True
        Archivo = "D:\excel\balance.general.xlsx" 'Pon aqui el nombre de archivo que quieras
        APrueba = a.Workbooks.Open(Archivo)
        ...
        With APrueba.WorkSheets("Hoja1")
            'ACTIVO CORRIENTE
            fila = 12
            dataActivo = obj.nCrudListarBD("select * from balance_general where grupo LIKE 'ACTIVO CORRIENTE%'", CadenaConexion)
            ...
            'ACTIVO NO CORRIENTE
            fila = 22
            dataActivo = obj.nCrudListarBD("select * from balance_general where grupo LIKE 'ACTIVO NO CORRIENTE%'", CadenaConexion)
            For i As Integer = 0 To dataActivo.Rows.Count - 1
                'MsgBox(dataActivo.Rows(i)("id").ToString & "-" & dataActivo.Rows(i)("item").ToString)
                Dim dataDet As New DataTable
                dataDet = obj.nCrudListarBD("select * from detalle_balance_general where id_bg='" & dataActivo.Rows(i)("id").ToString & "'", CadenaConexion)
            ...
            'PASIVO CORRIENTE
            fila = 12
            dataActivo = obj.nCrudListarBD("select * from balance_general where grupo LIKE 'PASIVO CORRIENTE%'", CadenaConexion)
            For i As Integer = 0 To dataActivo.Rows.Count - 1
                'MsgBox(dataActivo.Rows(i)("id").ToString & "-" & dataActivo.Rows(i)("item").ToString)
                Dim dataDet As New DataTable
            ...
            'PASIVO NO CORRIENTE
            fila = 20
            dataActivo = obj.nCrudListarBD("select * from balance_general where grupo LIKE 'PASIVO NO CORRIENTE%'", CadenaConexion)
            For i As Integer = 0 To dataActivo.Rows.Count - 1
                'MsgBox(dataActivo.Rows(i)("id").ToString & "-" & dataActivo.Rows(i)("item").ToString)
                Dim dataDet As New DataTable
                dataDet = obj.nCrudListarBD("select * from detalle_balance_general where id_bg='" & dataActivo.Rows(i)("id").ToString & "' order by id asc", CadenaConexion)
            ...
            'LIBRE
            fila = 28
            dataActivo = obj.nCrudListarBD("select * from balance_general where grupo LIKE 'LIBRE%'", CadenaConexion)
            For i As Integer = 0 To dataActivo.Rows.Count - 1
                'MsgBox(dataActivo.Rows(i)("id").ToString & "-" & dataActivo.Rows(i)("item").ToString)
                Dim dataDet As New DataTable
                dataDet = obj.nCrudListarBD("select * from detalle_balance_general where id_bg='" & dataActivo.Rows(i)("id").ToString & "' order by id asc", CadenaConexion)
            ...
            'PATRIMONIO NETO
            fila = 33
            dataActivo = obj.nCrudListarBD("select * from balance_general where grupo LIKE 'PATRIMONIO NETO%'", CadenaConexion)
            For i As Integer = 0 To dataActivo.Rows.Count - 1
                'MsgBox(dataActivo.Rows(i)("id").ToString & "-" & dataActivo.Rows(i)("item").ToString)
                Dim dataDet As New DataTable
                dataDet = obj.nCrudListarBD("select * from detalle_balance_general where id_bg='" & dataActivo.Rows(i)("id").ToString & "' order by id asc", CadenaConexion)
