...
Public Class frmTsUbigeo
    ...
    Private Sub cargarDepartamentos()
        ...
        Try
            data2 = obj.nCrudListar("select * from departamento order by nomdepartamento")
    ...
    Private Sub cargarProvincias()
        ...
        Dim data2 As DataTable
        Try
            data2 = obj.nCrudListar("select * from provincia order by nom_provincia")
            For Each row As DataRow In data2.Rows
                data.Rows.Add(row.Item(0).ToString, row.Item(1).ToString)
            Next
            With cboProvincia
    ...
    Private Sub cargarDistritos()
        ...
        Dim data2 As DataTable
        Try
            data2 = obj.nCrudListar("select * from distrito order by nom_distrito")
            For Each row As DataRow In data2.Rows
                data.Rows.Add(row.Item(0).ToString, row.Item(1).ToString)
            Next
            With cboDistrito
    ...
    Private Sub cargarDatos()
        Dim data As New DataTable
        data = obj.nCrudListar("select * from vista_ubigeo order by n_dep asc")
    ...
    Private Sub txtDato_TextChanged(sender As Object, e As EventArgs) Handles txtDato.TextChanged
        ...
        data = obj.nCrudListar("select * from vista_moneda_tipo_cambio where moneda like '%" & dato & "%'")
        dgvLista.DataSource = data
        cargaInicialEstados()
        'End If
    End Sub
    ...
    Private Sub cboDepartamento_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cboDepartamento.SelectedIndexChanged
        If iCarga = 1 Then
            ...
            Dim data2 As DataTable
            Try
                data2 = obj.nCrudListar("select * from provincia where coddepartamento='" & cboDepartamento.SelectedValue.ToString & "'")
                For Each row As DataRow In data2.Rows
                    data.Rows.Add(row.Item(0).ToString, row.Item(1).ToString)
                Next
                With cboProvincia
    ...
    Private Sub cboProvincia_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cboProvincia.SelectedIndexChanged
        If iCarga = 1 Then
            ...
            Dim data2 As DataTable
            Try
                data2 = obj.nCrudListar("select * from distrito where codprovincia='" & cboProvincia.SelectedValue.ToString & "' order by nom_distrito")
                For Each row As DataRow In data2.Rows
                    data.Rows.Add(row.Item(0).ToString, row.Item(1).ToString)
                Next
                With cboDistrito
