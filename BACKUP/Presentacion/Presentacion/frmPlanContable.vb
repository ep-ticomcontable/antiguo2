...
Public Class frmPlanContable
    ...
    Private Sub cargarPlanDeCuentas()
        dgvLista.DataSource = obj.nCrudListarBD("select * from vista_CuentaContableGrilla order by num_cuenta asc", CadenaConexion)
        lista = obj.nCrudListarBD("select * from vista_CuentaContableGrilla where nivel_cuenta<>'BALANCE' order by num_cuenta asc", CadenaConexion)
    End Sub
        ...
        If accion = "nuevo" Then
            ...
            Else
                ...
                If existeCuentaPadre.Rows.Count > 0 Then
                    campos = "id@sub_id@nombre@equi_sunat@bal_comprobacion@id_nivel_cta_cont@id_tipo_cta_cont@id_analisis_cta_cont@cuenta_debe@cuenta_haber@c1@id_banco@cuenta_corriente@id_moneda@estado"
                    valores = numCuenta & "@" & cuentaPadre & "@" & nomCuenta & "@" & ctaEquiv & "@" & balance & "@" & nivelCuenta & "@" & tipoCuenta & "@" & tipoAnalisis & "@" & ctaDebe & "@" & ctaHaber & "@" & estadoCC & "@" & banco & "@" & ctaCorriente & "@" & moneda & "@1"
                    Dim dato As String
    ...
    Private Sub generarExcel()
        ...
        Try
            ...
            a.Visible = True
            Archivo = "D:\excel\registro.plan.contable.xlsx" 'Pon aqui el nombre de archivo que quieras
