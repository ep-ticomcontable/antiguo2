...
Public Class frmTareasModulo
    ...
    Private Sub cargarPerfiles()
        ...
        data = obj.nCrudListar("select id,nombre,CASE WHEN estado = '0' THEN 'INACTIVO' ELSE 'ACTIVO' END AS estado from perfiles order by nombre asc")
    ...
    Private Sub cargarModulos()
        Dim data As New DataTable
        data = obj.nCrudListar("select id as 'id_modulo',nombre as 'nom_modulo' from modulos where estado=1 order by nombre asc")
    ...
    Private Sub cargarTareas()
        Dim data As New DataTable
        data = obj.nCrudListar("select id as 'id_tarea',nombre as 'nom_tarea' from tareas where estado=1 order by nombre asc")
    ...
    Private Sub cboProcesos_SelectedIndexChanged(sender As Object, e As EventArgs)
        If iCarga = 1 Then
            Dim data As New DataTable
            'data = obj.nCrudListar("usp_obtenerTareasPorModulo '" & cboProcesos.SelectedValue.ToString & "'")
    ...
    Private Sub cargarChecksModulos()
        If iCarga = 1 Then
            ...
            Dim data As New DataTable
            data = obj.nCrudListar("select id_modulo from perfil_modulos where id_perfil='" & idPerfil & "'")
    ...
    Private Sub cargarChecksTareas()
        If iCarga = 1 Then
            ...
            Dim data As New DataTable
            data = obj.nCrudListar("select id_modulo from proceso_modulos where id_perfil_modulo='" & idPerfil & "'")
