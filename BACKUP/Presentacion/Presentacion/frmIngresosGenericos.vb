...
Public Class frmIngresosGenericos
    ...
    Private Sub cargarMonedas()
        ...
        Dim data2 As DataTable
        Try
            data2 = obj.nCrudListar("select id,codigo,descripcion from tipo_moneda where estado=1 order by codigo asc")
            For Each row As DataRow In data2.Rows
    ...
    Private Sub cargarBancos()
        ...
        Dim data2 As DataTable
        Try
            data2 = obj.nCrudListar("select * from bancos order by id asc")
            For Each row As DataRow In data2.Rows
                data.Rows.Add(row.Item(0).ToString, row.Item(1).ToString)
            Next
            With cboBancos
                    ...
                    If Integer.Parse(row.Cells("id_centro").Value) > 0 Then
                        ...
                        If idCentroCosto > 0 Then
                            ...
                            For i As Integer = 0 To dtCC.Rows.Count - 1
                                ...
                                valores = data.Rows(0)("id").ToString() & "@" & dtCC.Rows(i)("cuenta").ToString & "@" & obtenerDatosCuenta(dtCC.Rows(i)("cuenta").ToString) & "@" & IIf(dtCC.Rows(i)("debe").ToString = "X", calculo, "0.00") & "@" & IIf(dtCC.Rows(i)("haber").ToString = "X", calculo, "0.00") & "@0"
                                Dim rptaCmdA As String = ""
                                rptaCmdA = obj.nCrudInsertarBD("@", "asientos_comodin", campos, valores, CadenaConexion)
                                If rptaCmdA <> "ok" Then
                                    MsgBox("Error al registrar Asiento Comodín: " & rptaCmdA)
